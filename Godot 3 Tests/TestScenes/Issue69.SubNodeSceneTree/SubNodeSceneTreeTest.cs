...
// Script with same name as scene

namespace GodotTests.TestScenes
{
    [SceneTree]
    public partial class SubNodeSceneTreeTest : Node, ITest
    {
        ...
        // Same as SubNodeScript.RunTests()
        private void RunTests()
        {
            ...
            void RootAccessTest()
            {
                ...
                root.GetChildren().Cast<Node>().Should().Equal([_.Node1, _.Node2, _.Node3, _.Node4]);
            ...
            void NavigationTest()
            {
                ...
                node1.GetChildren().Cast<Node>().Single().Should().Be(node1Child);
                node2.GetChildren().Cast<Node>().Single().Should().Be(node2Child);
                node3.GetChildren().Cast<Node>().Single().Should().Be(node3Child);
                node4.GetChildren().Cast<Node>().Single().Should().Be(node4Child);
            }
        }
    }
}
