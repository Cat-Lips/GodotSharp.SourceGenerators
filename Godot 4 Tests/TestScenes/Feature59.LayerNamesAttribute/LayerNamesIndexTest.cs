...
using SUT = (
    CanvasItem Render2D,
    VisualInstance3D Render3D,
    CollisionObject2D Physics2D,
    CollisionObject3D Physics3D,
    NavigationRegion2D Navigation2D,
    NavigationRegion3D Navigation3D,
    NavigationAgent2D Avoidance2D,
    NavigationAgent3D Avoidance3D);
...
public partial class LayerNamesIndexTest : Node, ITest
{
    ...
    {
        Test(WithLayer1Set, 1);
        Test(WithLayer2Set, 2);
        Test(WithLayer12Set, 3);
        Test(WithNoLayerSet, 0);

        static void Test(SUT sut, uint x, [CallerArgumentExpression(nameof(sut))] string name = null)
        {
            Test2D();
            Test3D();
